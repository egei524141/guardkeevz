...
if (args[0] == 'bilgi') {
  ...
  let salvo_user = await db.fetch(`whitelist_${user.id}`)
  if(salvo_user == true) {
  message.channel.send(salvoembed.setDescription(` ${user} İsimli Kullanıcı Güvenli Listede`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
  } else {
  message.channel.send(salvoembed.setDescription(` ${user} İsimli Kullanıcı Güvenli Listede Değil`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
  }
}
