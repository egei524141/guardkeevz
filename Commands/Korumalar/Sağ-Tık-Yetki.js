...
let sagtik = await message.channel.send(salvoembed.setDescription(`**__Sağ Tık Koruma - Yetki Engel;__**

Bu işlemi kabul ederseniz whitelistte olan kullanıcılar haricinde 
sağ tık kullanarak üyelere yetki veren herkes engellenecektir.
...
if (reaction.emoji.name === "✅") {
    sagtik.edit(salvoembed.setColor("RANDOM").setDescription(`Sağ Tık Koruma - Yetki Engel Başarılı Bir Şekilde Aktif Edildi.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    sagtik.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
  
    aktifEt();
} else if (reaction.emoji.name === "❎") {
    sagtik.edit(salvoembed.setColor("RANDOM").setDescription(`Sağ Tık Koruma - Yetki Engel Başarılı Bir Şekilde Pasif Bırakıldı.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    sagtik.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
...
const aktifEt = async () => {
    db.set(`${message.guild.id}_sagtik_yetki`, "aktif")
};

const pasifEt = async () => {
    db.delete(`${message.guild.id}_sagtik_yetki`)
};

};
exports.conf = {
  aliases: ['sag-tık-yetki','sağ-tık-yetki'],
  permLevel: 0
};
