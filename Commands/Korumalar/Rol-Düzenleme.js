...
let rolkoruma = await message.channel.send(salvoembed.setDescription(`**__Rol Koruma - Update Engel;__**

Bu işlemi kabul ederseniz whitelistte olan kullanıcılar haricinde 
rollar üzerinde değişiklik yapan herkes engellenecektir.
...
if (reaction.emoji.name === "✅") {
    rolkoruma.edit(salvoembed.setColor("RANDOM").setDescription(`Rol Koruma - Düzenleme Engel Başarılı Bir Şekilde Aktif Edildi.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    rolkoruma.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
    
    aktifEt();
} else if (reaction.emoji.name === "❎") {
    rolkoruma.edit(salvoembed.setColor("RANDOM").setDescription(`Rol Koruma - Düzenleme Engel Başarılı Bir Şekilde Pasif Bırakıldı.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    rolkoruma.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
...
const aktifEt = async () => {
    db.set(`${message.guild.id}_rolkoruma_update`, "aktif")
};

const pasifEt = async () => {
    db.delete(`${message.guild.id}_rolkoruma_update`)
};

};
exports.conf = {
  aliases: ['rol-koruma-update'],
  permLevel: 0
};

exports.help = {
  name: 'rol-koruma-update',
  description: 'Salvatore was here',
  usage: 'rol-koruma-update'
};