...
let sagtik = await message.channel.send(salvoembed.setDescription(`**__Sağ Tık Koruma - Ban Engel;__**

Bu işlemi kabul ederseniz whitelistte olan kullanıcılar haricinde 
sağ tık kullanarak üye banlayan herkes engellenecektir.
...
if (reaction.emoji.name === "✅") {
    sagtik.edit(salvoembed.setColor("RANDOM").setDescription(`Sağ Tık Koruma - Ban Engel Başarılı Bir Şekilde Aktif Edildi.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    sagtik.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
    aktifEt();
} else if (reaction.emoji.name === "❎") {
    sagtik.edit(salvoembed.setColor("RANDOM").setDescription(`Sağ Tık Koruma - Ban Engel Başarılı Bir Şekilde Pasif Bırakıldı.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    sagtik.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
...
const aktifEt = async () => {
    db.set(`${message.guild.id}_sagtik_ban`, "aktif")
};

const pasifEt = async () => {
    db.delete(`${message.guild.id}_sagtik_ban`)
};

};
exports.conf = {
  aliases: ['sag-tık-ban','sağ-tık-ban'],
  permLevel: 0
};
