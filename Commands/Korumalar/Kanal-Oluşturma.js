...
let kanalkoruma = await message.channel.send(salvoembed.setDescription(`**__Kanal Koruma - Oluşturma Engel;__**

Bu işlemi kabul ederseniz whitelistte olan kullanıcılar haricinde 
kanal oluşturan herkes engellenecektir.
...
if (reaction.emoji.name === "✅") {
    kanalkoruma.edit(salvoembed.setColor("RANDOM").setDescription(`Kanal Koruma - Oluşturma Engel Başarılı Bir Şekilde Aktif Edildi.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    kanalkoruma.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
  
    aktifEt();
} else if (reaction.emoji.name === "❎") {
    kanalkoruma.edit(salvoembed.setColor("RANDOM").setDescription(`Kanal Koruma - Oluşturma Engel Başarılı Bir Şekilde Pasif Bırakıldı.`)).then(m => m.delete({timeout: Config.Embed.Timeout}));
    kanalkoruma.reactions.removeAll().catch(error => console.error("Bir Hata Oluştu: : ", error));
...
const aktifEt = async () => {
    db.set(`${message.guild.id}_kanalkoruma_create`, "aktif")
};

const pasifEt = async () => {
    db.delete(`${message.guild.id}_kanalkoruma_create`)
};

};
exports.conf = {
  aliases: ['kanal-koruma-create'],
  permLevel: 0
};

exports.help = {
  name: 'kanal-koruma-create',
  description: 'Salvatore was here',
  usage: 'kanal-koruma-create'
};